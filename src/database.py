...
from sqlalchemy import (
    ...
    Text, JSON, ForeignKey, Enum as SQLEnum, Numeric, Index, func
)
...
class AutomationTask(Base):
    ...
    def efficiency_gain_percentage(self) -> float:
        ...
        if self.manual_duration_minutes > 0:
            ...
            return ((self.manual_duration_minutes - automated_minutes) / self.manual_duration_minutes) * 100
        return 0
    
    def calculate_savings(self, hourly_rate: Optional[float] = None) -> Tuple[float, Decimal]:
        """
        ...
        self.manual_cost = (Decimal(str(self.manual_duration_minutes)) / 60) * self.hourly_rate
        ...
        self.automated_cost = (Decimal(str(self.automated_duration_seconds)) / 3600) * automation_overhead_rate
...
class PerformanceMetrics(Base):
    ...
    def conversion_improvement(self) -> float:
        ...
        if self.baseline_conversion_rate and self.baseline_conversion_rate > 0:
            return ((self.conversion_rate - self.baseline_conversion_rate) / self.baseline_conversion_rate) * 100
        return 0
...
class ROITracking(Base):
    ...
    performance_value_added = Column(Numeric(10, 2), default=0)  # Value from improved performance
    ...
    def calculate_roi(self):
        ...
        if self.automation_cost > 0:
            self.roi_percentage = (float(self.net_benefit) / float(self.automation_cost)) * 100
        else:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'labor_cost_saved': float(self.labor_cost_saved) if self.labor_cost_saved else 0,
            'performance_value_added': float(self.performance_value_added) if self.performance_value_added else 0,
            'total_cost_saved': float(self.total_cost_saved) if self.total_cost_saved else 0,
            'avg_ctr_improvement': self.avg_ctr_improvement,
            'avg_conversion_improvement': self.avg_conversion_improvement,
            'revenue_increase': float(self.revenue_increase) if self.revenue_increase else 0,
            'automation_cost': float(self.automation_cost) if self.automation_cost else 0,
...
class AIDecisionHistory(Base):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'impact_measured_at': self.impact_measured_at.isoformat() if self.impact_measured_at else None,
...
class DatabaseManager:
    ...
    def calculate_period_roi(
        ...
        with self.get_session() as session:
            ...
            ctr_improvements = [p.ctr_improvement for p in performances if p.ctr_improvement]
            conversion_improvements = [p.conversion_improvement for p in performances if p.conversion_improvement]
            
            avg_ctr_improvement = sum(ctr_improvements) / len(ctr_improvements) if ctr_improvements else 0
            avg_conversion_improvement = sum(conversion_improvements) / len(conversion_improvements) if conversion_improvements else 0
            
            # Estimate performance value added (simplified calculation)
            performance_value_added = Decimal('0')
            for perf in performances:
                if perf.baseline_conversion_rate and perf.conversion_rate > perf.baseline_conversion_rate:
                    additional_conversions = ((perf.conversion_rate - perf.baseline_conversion_rate) / 100) * perf.clicks
                    # Assume average order value of $100
                    performance_value_added += Decimal(str(additional_conversions * 100))
    ...
    def get_automation_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            total_tasks = session.query(func.count(AutomationTask.id)).filter(
            ...
            total_time_saved = session.query(
                func.sum(AutomationTask.manual_duration_minutes - 
            ...
            # Total cost saved
            total_cost_saved = session.query(
                    ...
                    case(
                        (AutomationTask.manual_duration_minutes > 0,
                         ((AutomationTask.manual_duration_minutes - 
                           (AutomationTask.automated_duration_seconds / 60)) / 
                          AutomationTask.manual_duration_minutes) * 100),
                        else_=0
                    )
                )
            ).filter(
            ...
            successful_decisions = session.query(func.count(AIDecisionHistory.id)).filter(
                AIDecisionHistory.decision_timestamp >= start_date,
                AIDecisionHistory.success_score >= 80
            ).scalar()
            
            total_decisions = session.query(func.count(AIDecisionHistory.id)).filter(
                AIDecisionHistory.decision_timestamp >= start_date,
            ...
            return {
                ...
                'ai_decision_success_rate': (successful_decisions / total_decisions) * 100,
