...
5. Alternative scenarios if applicable""",
                variables=["campaigns", "total_budget", "optimization_goal", "constraints", "historical_data"],
...
4. Optimize for the platform's best practices""",
                variables=["product_name", "product_description", "target_audience", "key_benefits", 
                          "tone", "platform", "headline_limit", "description_limit", "num_variants",
                          "audience_insights", "competitor_ads"],
...
5. Estimated reach and impact""",
                variables=["audience_data", "objective", "product_info", "budget_range", "segment_performance"],
...
5. Risk factors to monitor""",
                variables=["historical_data", "seasonality_data", "market_trends", 
                          "competitive_data", "prediction_period"],
    ...
    async def generate_optimization_suggestions(
                ...
                "properties": {
                    "suggestions": {
                        ...
                        "items": {
                            "type": "object",
                            "properties": {
                                "type": {"type": "string", "enum": ["budget_reallocation", "bid_adjustment", "targeting_change", "creative_refresh", "pause_campaign"]},
                                "campaign_id": {"type": "string"},
                                "action": {"type": "string"},
                                "predicted_impact": {
                                    "type": "object",
                            ...
                            "required": ["type", "campaign_id", "action", "predicted_impact", "confidence", "reasoning"]
                        }
                    },
                    "budget_allocation": {
                        "type": "object",
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a marketing optimization expert specializing in budget allocation and campaign performance."},
                    {"role": "user", "content": prompt}
                ],
                functions=functions,
    ...
    async def create_personalized_ad_copy(
        ...
        key_benefits = ["High quality", "Cost-effective", "Easy to use"]  # Could use NLP to extract
                    ...
                    "variants": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "tone": {"type": "string"},
                                "predicted_ctr": {"type": "number", "minimum": 0, "maximum": 100},
                                "keywords": {"type": "array", "items": {"type": "string"}},
                                "unique_selling_points": {"type": "array", "items": {"type": "string"}}
                            },
                            "required": ["headline", "description", "call_to_action", "predicted_ctr"]
                        }
                    },
                    "recommendations": {
                        "type": "object",
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert copywriter specializing in digital advertising."},
                    {"role": "user", "content": prompt}
                ],
                functions=functions,
            ...
            for variant in function_args["variants"]:
                variants.append(AdCopyVariant(
                    headline=variant["headline"],
                    description=variant["description"],
                    call_to_action=variant["call_to_action"],
                    target_audience=variant.get("target_audience", str(target_audience)),
    ...
    async def execute_prompt_chain(
        ...
        for i, step in enumerate(steps):
            ...
            if template_name == "campaign_analysis":
                ...
                result = await self._execute_generic_prompt(prompt, step.get("output_format"))
            
            # Store step result
            step_result = {
                "step_name": step_name,
                "template": template_name,
                "result": result if not isinstance(result, list) else [asdict(r) if hasattr(r, '__dataclass_fields__') else r for r in result]
            }
    ...
    async def _execute_generic_prompt(self, prompt: str, output_format: Optional[Dict[str, Any]] = None) -> Any:
        """Execute a generic prompt with optional structured output"""
        messages = [
            {"role": "system", "content": "You are a marketing automation AI assistant."},
            {"role": "user", "content": prompt}
        ]
        ...
        chain_steps = [
            ...
            },
            {
                ...
                "variables": {
                    ...
                    "total_budget": total_budget,
                    "optimization_goal": business_goals.get("primary_goal", "maximize_roi"),
...
async def test_ai_engine():
    ...
    ad_variants = await engine.create_personalized_ad_copy(
        ...
        target_audience={"age": "25-45", "interests": ["marketing", "technology"], "job_title": "marketing manager"},
        platform="google_ads",
        num_variants=3
    )
