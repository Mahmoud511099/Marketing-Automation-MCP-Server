...
class GoogleAnalyticsClient(BaseIntegrationClient):
    ...
    async def authenticate(self):
        ...
        if self.service_account_path and os.path.exists(self.service_account_path):
            ...
            raise AuthenticationError("No valid authentication method found for Google Analytics")
    ...
    async def fetch_campaign_performance(
        ...
        for row in response.get("rows", []):
            ...
            result = {
                "campaign_id": dimension_values[0].get("value") if len(dimension_values) > 0 else "unknown",
                "campaign_name": dimension_values[1].get("value") if len(dimension_values) > 1 else "unknown",
                "date": dimension_values[2].get("value") if len(dimension_values) > 2 else "unknown",
                "metrics": {}
            }
            ...
            for i, metric in enumerate(metrics):
                if i < len(metric_values):
                    ...
                    try:
                        if "rate" in metric or metric in ["bounce_rate", "conversion_rate"]:
    ...
    async def update_campaign_budget(
        ...
        logger.warning("Google Analytics cannot update campaign budgets. Use Google Ads or Facebook Ads APIs.")
    ...
    async def get_audience_insights(
        ...
        for row in response.get("rows", []):
            dimensions_data = {dim["name"]: val["value"] 
                             for dim, val in zip(dimensions, row.get("dimensionValues", []))}
            metrics_data = {metric["name"]: float(val.get("value", 0)) 
            ...
            insights["geographic_distribution"][country]["users"] += metrics_data.get("totalUsers", 0)
            insights["geographic_distribution"][country]["sessions"] += metrics_data.get("sessions", 0)
            
            # Aggregate by device
            device = dimensions_data.get("deviceCategory", "Unknown")
            if device not in insights["device_distribution"]:
                insights["device_distribution"][device] = {
                    "users": 0,
                    "sessions": 0
                }
            insights["device_distribution"][device]["users"] += metrics_data.get("totalUsers", 0)
            insights["device_distribution"][device]["sessions"] += metrics_data.get("sessions", 0)
            ...
            if "userAgeBracket" in dimensions_data:
                ...
                if age_bracket not in insights["demographic_distribution"]["age_distribution"]:
                    insights["demographic_distribution"]["age_distribution"][age_bracket] = 0
                insights["demographic_distribution"]["age_distribution"][age_bracket] += metrics_data.get("totalUsers", 0)
                
                if gender not in insights["demographic_distribution"]["gender_distribution"]:
                    insights["demographic_distribution"]["gender_distribution"][gender] = 0
                insights["demographic_distribution"]["gender_distribution"][gender] += metrics_data.get("totalUsers", 0)
        
        # Calculate overall behavioral metrics
        total_users = sum(row.get("metricValues", [{}])[0].get("value", 0) 
                         for row in response.get("rows", []))
        
        insights["behavioral_metrics"] = {
            "total_users": total_users,
            "avg_engagement_rate": response.get("totals", [{}])[3].get("value", 0) if len(response.get("totals", [])) > 3 else 0,
            "avg_session_duration": response.get("totals", [{}])[4].get("value", 0) if len(response.get("totals", [])) > 4 else 0
        }
